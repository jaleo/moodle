...
class theme_cafe_con_leche_core_renderer extends core_renderer {
    ...
    public function footer() {
        ...
        if (defined('MDL_PERF') || (!empty($CFG->perfdebug) and $CFG->perfdebug > 7)) {
            ...
            if (defined('MDL_PERFTOFOOT') || debugging() || $CFG->perfdebug > 7) {
                $performanceinfo = cafe_con_leche_performance_output($perf);
            }
        }
    ...
    protected function render_custom_menu(custom_menu $menu) {
        ...
        $content = html_writer::start_tag('ul', array('class'=>'dropdown dropdown-horizontal cafe_con_leche-custom-menu'));
        // Render each child
...
class theme_cafe_con_leche_topsettings_renderer extends plugin_renderer_base {
            ...
            if($isbranch && $item->children->count()==0) {
                ...
                if (!empty($CFG->navshowallcourses) && $item->key === 'courses') {
                    if(!$coursessubnav) {
                        ...
                        $dummypage = new cafe_con_leche_dummy_page();
                        $dummypage->set_context($PAGE->context);
                        $dummypage->set_url($PAGE->url);
                        $coursessubnav = new cafe_con_leche_expand_navigation($dummypage, $item->type, $item->key);
                        $expanded = true;
                    }
                    ...
                    if(!$mainsubnav) {
                        ...
                        $dummypage->set_context($PAGE->context);
                        $dummypage->set_url($PAGE->url);
                        $mainsubnav = new cafe_con_leche_expand_navigation($dummypage, $item->type, $item->key);
                        $expanded = true;
                    }
                    $subnav = $mainsubnav;
                }
    ...
    public function search_form(moodle_url $formtarget, $searchvalue) {
		...
		$por_defecto = get_string('searchsettings', 'theme_cafe_con_leche');
        if (empty($searchvalue)) {
            $searchvalue = $por_defecto;
        }
        ...
        $content .= html_writer::empty_tag('input', array('id' => 'topadminsearchquery', 'type' => 'text', 'name' => 'query', 'value' => s($searchvalue),
                    'onfocus' => "if(this.value == '".$por_defecto."') {this.value = '';}",
                    'onblur' => "if (this.value == '') {this.value = '".$por_defecto."';}"));
...
class theme_cafe_con_leche_core_course_renderer extends core_course_renderer {
