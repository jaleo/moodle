 ...
 * @return string
 */
function cafe_con_leche_performance_output($param) {
 ...
 * @return string
 */
function cafe_con_leche_process_css($css, $theme) {
    ...
    $css = cafe_con_leche_set_backgroundcolor($css, $backgroundcolor);
	
	//Proceso el color de fondo de la cabecera
	if (!empty($theme->settings->cabecerabackgroundcolor)) {
        $backgroundcolor = $theme->settings->cabecerabackgroundcolor;
    } else {
        $backgroundcolor = null;
    }
    $css = cafe_con_leche_set_cabecerabackgroundcolor($css, $backgroundcolor);

	//Proceso el logotipo
	 // $logo = $theme->setting_file_url('logotipo', 'logotipo');
	 // $css = cafe_con_leche_set_logo($css, $logo, $theme);
    ...
    $css = cafe_con_leche_set_customcss($css, $customcss);
...
//Establece el logotipo
//Copiado del tema afterburner
function cafe_con_leche_set_logo($css, $logo, $theme) {
    $tag = '[[setting:logotipo]]';
    $replacement = $logo;
    if (is_null($replacement)) {
        $replacement = $theme->setting_file_url($setting, $setting);
    }
 ...
 * @return string
 */
function cafe_con_leche_set_backgroundcolor($css, $backgroundcolor) {
...
/**
 * Sets the headers background colour variable in CSS.
 * Establede el color de la cabecera en la variable CSS.
 *
 * @param string $css
 * @param mixed $backgroundcolor
 * @return string
 */
function cafe_con_leche_set_cabecerabackgroundcolor($css, $backgroundcolor) {
    $tag = '[[setting:cabecerabackgroundcolor]]';
    $replacement = $backgroundcolor;
    if (is_null($replacement)) {
        $replacement = '#BEBA82';
    }
 ...
 * @return string
 */
function cafe_con_leche_set_customcss($css, $customcss) {
 ...
 *     theme/cafe_con_leche/yui/editbuttons/editbuttons.js
 *
 * @param moodle_page $page 
 */
function cafe_con_leche_initialise_editbuttons(moodle_page $page) {
    global $CFG;
    ...
    $page->requires->yui_module('moodle-theme_cafe_con_leche-editbuttons', 'M.theme_cafe_con_leche.initEditButtons');
    return true;
}

function cafe_con_leche_initialise_awesomebar(moodle_page $page) {
    ...
    $page->requires->yui_module('moodle-theme_cafe_con_leche-awesomebar', 'M.theme_cafe_con_leche.initAwesomeBar');
}

function cafe_con_leche_require_course_login($courseorid, $autologinguest = true, $cm = NULL, $setwantsurltome = true, $preventredirect = true) {
    global $CFG, $SITE;
    ...
    if (!empty($CFG->forcelogin)) {
        ...
        cafe_con_leche_require_login($courseorid, $autologinguest, $cm, $setwantsurltome, $preventredirect);
...
function cafe_con_leche_require_login($courseorid = NULL, $autologinguest = true, $cm = NULL, $setwantsurltome = true, $preventredirect = false) {
...
class cafe_con_leche_expand_navigation extends global_navigation {
    ...
    public function expand($branchtype, $id) {
        global $CFG, $DB, $PAGE;
        static $cafe_con_leche_course_activities = array();
                ...
                try {
                    if(!array_key_exists($course->id, $this->expandedcourses)) {
                        ...
                        if ($PAGE->course->id == $course->id && (!method_exists($this, 'format_display_course_content') || $this->format_display_course_content($course->format))) {
                            cafe_con_leche_require_course_login($course);
            ...
            case self::TYPE_SECTION :
                ...
                try {
                    ...
                    if (property_exists($PAGE->theme->settings, 'expandtoactivities') && $PAGE->theme->settings->expandtoactivities) {
                        if(!array_key_exists($course->id, $cafe_con_leche_course_activities)) {
                            list($sectionarray, $activities) = $this->generate_sections_and_activities($course);
                            $cafe_con_leche_course_activities[$course->id] = $activities;
                        }
                        $sections = $this->expandedcourses[$course->id];
                        $activities = $cafe_con_leche_course_activities[$course->id];
...
class cafe_con_leche_dummy_page extends moodle_page {
    /**
...
/**
 * Sirve los ficheros de cafe_con_leche.
 * Adaptado de Formal White.
 *
 * @param stdClass $course
 * @param stdClass $cm
 ...
 * @return bool
 */
function theme_cafe_con_leche_pluginfile($course, $cm, $context, $filearea, $args, $forcedownload, array $options = array()) {
    if ($context->contextlevel == CONTEXT_SYSTEM and $filearea === 'logotipo') {
        $theme = theme_config::load('cafe_con_leche');
        return $theme->setting_file_serve('logotipo', $args, $forcedownload, $options);
    } else if ($context->contextlevel == CONTEXT_SYSTEM and $filearea === 'fondo_cabecera') {
        $theme = theme_config::load('cafe_con_leche');
        return $theme->setting_file_serve('fondo_cabecera', $args, $forcedownload, $options);
